...
def _process_trajectory_worker_traffic(
    ...
    try:
        ...
        # 在转换前，保存元数据列，因为 traffic 库会丢弃它们
        meta_cols = ["PARTNO", "P1", "GP", "TASK", "PLANETYPE"]
        meta_df = trajectory_df_polars.select(meta_cols).head(1) # 只需第一行作为代表
...
def process_flight_data(
    ...
    min_len_for_clean: int,
    min_valid_block_len: int,
    unique_id_strategy: str = "timestamp",
    ...
    ).select(["ID", "Time", "Lon", "Lat", "H", "PLANETYPE", "PARTNO", "GP", "TASK"])
    ...
    if "unique_id_strategy" in locals() and unique_id_strategy == "numeric":
    ...
    else: # Default to timestamp strategy
        logging.info(
            "使用 'timestamp' 策略生成轨迹ID (例如: PlaneA_YYYYMMDD_HHMMSS)"
        )
        # 获取每个航段的第一个时间点
        trajectory_start_time = pl.col("Time").first().over("ID", "segment_id")
        # 将起始时间格式化为 YYYYMMDD_HHMMSS
        time_str = trajectory_start_time.dt.strftime("%Y%m%d_%H%M%S")

        segmented_lf = segmented_lf.with_columns(
            (pl.col("ID").cast(pl.Utf8) + "_" + time_str).alias("Unique_ID")
        )
    ...
    try:
        ...
        final_segmented_lf = segmented_lf.select(
            "Unique_ID", "Time", "Lon", "Lat", "H", "PLANETYPE", "PARTNO", "GP", "TASK", pl.col("ID").alias("P1")
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help='生成轨迹唯一ID的策略: "numeric" (例如: PlaneA_0) 或 "timestamp" (例如: PlaneA_YYYYMMDD_HHMMSS)',
    )
    parser.add_argument(
